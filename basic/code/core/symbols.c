...
void initialise_symbol_table( void )
{
	...
	// initialise the heap (base & top) pointers to be just above any stored program
	heap_base         = top + 1;
	heap_top          = top + 1;
	
	// now initialise the symbol table (base) pointer to be at the start of the heap
	// and then mark the symbol table as empty
	symbol_table_base  = top + 1;
...
void _add_to_symbol_table( unsigned char *name , unsigned char type , union symbol_table_entry_value value )
{
	...
	switch ( type )
	{
		...
		case DATATYPE_REAL    :
			...
			record_length = 2 + strlen((char *)name)+1 + 4;
			*(ptr+0) = record_length;
			*(ptr+1) = DATATYPE_REAL;
			strcpy( (char *)ptr+2 , (char *)name );
			* (float *)(ptr+2+strlen((char *)name)+1+0) = value.rval;
			break;
		...
		case DATATYPE_INTEGER :
			...
			*(ptr+1) = DATATYPE_INTEGER;
			strcpy( (char *)ptr+2 , (char *)name );
			* (int *)(ptr+2+strlen((char *)name)+1+0) = value.ival;
			break;
			
		case DATATYPE_STRING :
		
			record_length = 2 + strlen((char *)name)+1 + strlen((char *)value.sval)+1;
			*(ptr+0) = record_length;
			*(ptr+1) = DATATYPE_STRING;
			strcpy( (char *)ptr+2 , (char *)name );
			strcpy( (char *)ptr+2+strlen((char *)name)+1 , (char *)value.sval );
			break;
		...
		case DATATYPE_PROCEDURE :
			...
			*(ptr+1) = DATATYPE_PROCEDURE;
			strcpy( (char *)ptr+2 , (char *)name );
			*(ptr+2+strlen((char *)name)+1+0) = (unsigned char) (value.pval >>  0);		
			*(ptr+2+strlen((char *)name)+1+1) = (unsigned char) (value.pval >>  8);
			*(ptr+2+strlen((char *)name)+1+2) = (unsigned char) (value.pval >> 16);
			*(ptr+2+strlen((char *)name)+1+3) = (unsigned char) (value.pval >> 24);
			break;
		...
		case DATATYPE_BOOLEAN :
			...
			record_length = 2 + strlen((char *)name)+1 + 1;
			*(ptr+0) = record_length;
			*(ptr+1) = DATATYPE_BOOLEAN;
			strcpy( (char *)ptr+2 , (char *)name );
			*(ptr+2+strlen((char *)name)+1+0) = value.bval;		
			break;
...

{
	...
	while ( keep_looking )
	{
		...
		{
			if ( strncmp( (const char *)identifier , (const char *)ptr+2 , strlen((char *)identifier) ) == 0x00 )
			{
				// ok - identifier found - but is its definition followed by a terminator? (":" or a NUL)
				c = *( ptr + 2 + strlen((char *)identifier) );
				if ( c == ':' || !c )
				{
					position     = index;
					keep_looking = 0;
...
union symbol_table_entry_value _symbol_table_entry_value( int position , unsigned char *base )
{
	...
	// ok - we've now located the base pointer to the element in the symbol table - no reconstitute its value/structure
	record_length = *ptr++;
	data_type     = *ptr++;
	// skip the identifier (including the terminating element)
	while ( *ptr )
		ptr++;
	ptr++;
...
void symbol_table_entry_identifier( unsigned char *identifier , int position )
{
	...
	strcpy( (char *)identifier , (char *)ptr+2 );
