 ...
 * 					returns the result of the SIN function using 'x' as the parameter where x is a measurement in degrees.
 * 
 */
void _function_sin( void )
{
	...
	// SIN requires a single-element parameter list (containing an INTEGER).
	...
	if ( !expect_token( PUNC_CLOSE_BRACKET ) )
 	...
 	switch ( type )
 	{
 		case DATATYPE_INTEGER : 
 			...
 			operand.rval = sin( (double) (operand.ival) * 3.142 / 180 );
 			push_operand_real( operand.rval );          
 			break;
 		case DATATYPE_REAL : 
 			operand.rval = pop_operand_real();
 			operand.rval = sin( (double) (operand.rval) * 3.142 / 180 );
 			push_operand_real( operand.rval );          
 			break;
 		default               : 
